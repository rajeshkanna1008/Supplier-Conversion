...
-----------------------------3rd Corsor for Contacts  ----------------------------------
       CURSOR lcu_contacts( p_vendor_id NUMBER,p_vendor_site_code VARCHAR2) IS
          SELECT xascs.ROWID , xascs.*
...
--****************** Validation Start for Suppliers ***************************
            ...
            IF sup_rec.terms_name IS NOT NULL THEN
                       ...
                       EXCEPTION
                           WHEN NO_DATA_FOUND THEN
                            l_suppliers_error:='Y';
                            l_sup_err_details := l_sup_err_details||'\ Term Name -'|| sup_rec.terms_name
                            ||' does not Exists for supplier '||sup_rec.supplier_name;
                           fnd_file.put_line (fnd_file.LOG,l_sup_err_details);
                          WHEN OTHERS  THEN
                                 l_suppliers_error:='Y';
                                 l_sup_err_details := l_sup_err_details||'\ Terms Name Validation failed for  Term Name -'
                  ...
                  END;
                    ELSE
                            l_suppliers_error:='Y';
                               l_sup_err_details := l_sup_err_details||'\  Term Name can not be blank for supplier '
            ...
            --=================================================================
               ...
               --==================================================================
                    ...
                    EXCEPTION
                           ...
                           THEN
                          l_suppliers_error:='Y';
                          l_sup_err_details := l_sup_err_details || ' \ Payment Currency Code -'
                            || sup_rec.Payment_currency
                            ||' does not Exists for supplier '||sup_rec.supplier_name;
                       WHEN OTHERS
                       THEN
                          l_suppliers_error:='Y';
                          l_sup_err_details := l_sup_err_details||'\ Supplier Payment currency Validation failed for  currency code -'
        ...
        IF sup_rec.pay_group IS NOT NULL THEN
            ...
            EXCEPTION
                   WHEN NO_DATA_FOUND THEN
                          l_suppliers_error:='Y';
                          l_sup_err_details := l_sup_err_details || ' \ Pay Group Code -'
                        || sup_rec.Pay_group
                        ||' does not Exists for supplier '
                        ||sup_rec.supplier_name;
                   WHEN OTHERS THEN
                          l_suppliers_error:='Y';
                           l_sup_err_details := l_sup_err_details||'\ Supplier Pay Group Validation failed for  pay Group -'
                   ...
                   --==================================================================
                    ...
                    and UPPER(payment_method_name) = UPPER(sup_rec.payment_method);
                        EXCEPTION
                               ...
                               WHEN OTHERS THEN
                                  l_suppliers_error:='Y';
                           l_sup_err_details :=   l_sup_err_details||'\ Supplier Payment Method Validation'
                                ...
                                WHERE UPPER(name)=UPPER(sit_rec.terms_name);
                                 EXCEPTION
                                   ...
                                   fnd_file.put_line (fnd_file.LOG,l_sit_err_details);
                                       WHEN OTHERS  THEN
                                         l_suppliers_error:='Y';
                                         l_sit_err_details := l_sit_err_details||'\ Terms Name Validation failed in Supplier Sites'
                        ...
                        BEGIN
                                ...
                                ||sup_rec.supplier_name||'/'||sit_rec.country;
                                   fnd_file.put_line (fnd_file.LOG,l_sit_err_details);
                                       WHEN OTHERS  THEN
                                         l_suppliers_error:='Y';
                                         l_sit_err_details := l_sit_err_details||'\ Country Exception failed in Supplier Sites'
                                    || 'for Supplier/Site : '
                                    ||sup_rec.supplier_name||'/'
                                    ||sit_rec.country
                                    ||' Error no -'
            ...
            --=================================================================
               ...
               --==================================================================
                    ...
                    EXCEPTION
                       ...
                       THEN
                          l_sites_error:='Y';
                          l_sit_err_details := l_sit_err_details||'\ Invoice Currency exception failed for' 
            ...
            --=================================================================
               ...
               --==================================================================
                    ...
                    EXCEPTION
                       ...
                       THEN
                          l_sites_error:='Y';
                          l_sit_err_details := l_sit_err_details||'\ payment Currency exception failed for' 
            ...
            --+**************** Validation end for Sites ****************
                ...
                --=================================================================================
                 ...
                 ELSE
                    ...
                    EXCEPTION
                        WHEN NO_DATA_FOUND THEN
                              l_con_err_details  := l_con_err_details ||'\ Operating Unit not Defined For supplier/Site/operating unit'
                            ||sup_rec.supplier_name||'/'||sit_rec.site_name||'/'
